...
# Exibir confirmação
echo -e "\n ${green}Versão antiga: $oldpost \n \n Versão nova: $newpost \n \n Diretório atual: $currentDir ${reset} \n"
...
# Criar diretórios
mkdir /home/postgres$newpost &> /dev/null
mkdir /home/postgres$newpost/data &> /dev/null

# Ajustar proprietário
chown -R postgres:postgres /home/postgres$newpost &> /dev/null

# Copiar dados do binario padrão para o novo
cp -Rf /var/lib/postgresql/$newpost/main/* /home/postgres$newpost/data &> /dev/null

# Navegar até o diretório
cd /home/postgres$newpost
...
/usr/lib/postgresql/$oldpost/bin/psql -d banco -U USERDONO -c "ALTER ROLE postgres WITH SUPERUSER;" &> /dev/null
...
sed -i -e "s|data_directory = '/var/lib/postgresql/14/main'|data_directory = '/home/postgres$newpost/data/'|" postgresql.conf
...
touch /home/postgres$newpost/data/pg_upgrade_server.log &> /dev/null
chown -R postgres:postgres * &> /dev/null
chown -R postgres:postgres /home/postgres$newpost/data/pg_upgrade_server.log &> /dev/null
chmod 700 * &> /dev/null
chmod 777 /home/postgres$newpost/data/pg_upgrade_server.log &> /dev/null
...
cp /etc/postgresql/$newpost/main/postgresql.conf /home/postgres$newpost/data/ &> /dev/null
cp -r /etc/postgresql/$newpost/main/conf.d /home/postgres$newpost/data/ &> /dev/null
cd /home/postgres$newpost/data/ &> /dev/null
...
/usr/lib/postgresql/$newpost/bin/pg_ctl stop -D "/home/postgres$newpost/data" &> /dev/null

cd /home/postgres$newpost/data/
echo "${green} Iniciando processo de upgrade ${reset}"
echo " ${red} #### POR SEGURANÇA, NÃO STOPAR ESSE PROCESSO #### ${reset}"
/usr/lib/postgresql/$newpost/bin/pg_upgrade -b "/usr/lib/postgresql/$oldpost/bin" -B "/usr/lib/postgresql/$newpost/bin" -d "$currentDir" -D "/home/postgres$newpost/data" -Upostgres #Aparecer na tela o resultado
EOF
...
cd /home/postgres$newpost/data
/usr/lib/postgresql/$newpost/bin/psql -d banco -U USERDONO -f update_extensions.sql &> /dev/null
/usr/lib/postgresql/$newpost/bin/psql -d banco -U USERDONO -c "ALTER ROLE postgres WITH NOSUPERUSER;" &> /dev/null
